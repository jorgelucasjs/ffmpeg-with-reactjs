...
function App() {
	...
	const createVideo = async ()=>{
		...
		.then(async (result)=> {
			...
			ffmpeg.FS('writeFile', 'sometrack.wav', await fetchFile(soundFile as File));

			await ffmpeg.run("-i", "image.png", "-i", "sometrack.wav", "-c:a", 
			"libmp3lame", "-b:a", "256k", "-muxdelay", "0", "-f", "segment", 
			"-sc_threshold", "0", "-segment_time", "7", "-segment_list","playlist.m3u8","-segment_format","mpegts","","");
