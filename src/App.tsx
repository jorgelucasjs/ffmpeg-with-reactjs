...
function App() {
	...
	const createVideo = async ()=>{
		...
		.then(async ()=> {
			...
			//ffmpeg -y -i Lazy_Days_320_preview.mp3 -c:a aac -b:a 128k -muxdelay 0 -f segment -sc_threshold 0 -segment_time 7 -segment_list "playlist.m3u8" -segment_format -hls_list_size "file%d.m4a"

			const data = ffmpeg.FS('readFile', 'song.mp3');

			const audio = URL.createObjectURL(new Blob([data.buffer], {type: 'audio/mp3'}));
			setVideoSrc(audio);
		}).catch((error)=>{
			console.log('ERRO ====> ', error);
		});
	...
	return (
		<div className="App">
			<div>
				<audio src={videoSrc} controls></audio>
			</div>
			<div>
